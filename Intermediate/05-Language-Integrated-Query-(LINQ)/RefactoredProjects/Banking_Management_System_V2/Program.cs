...
try
{
    ...
    bool removeSuccess = bank.CloseAccount(accountNumber1);
    ...
    BankAccount account = bank.GetAccount(accountNumber2);
    account.DisplayInfo();

    // Try withdrawal method in fixed deposit account
    //fixedDeposit.Withdraw(300m);

    Console.WriteLine("Daily Total");
    foreach (var total in savings.GetDailyTotals())
    {
        Console.WriteLine($"{total.Day} | {total.Total:C} | ({total.Count})");
    }
}
